...
corpusPOS <- Corpus(VectorSource(stemmedPOS)) # turn into corpus
```
```{r freq2, echo=FALSE}
...
tdmPOS <- tm::DocumentTermMatrix(corpusPOS) 
...
tfidfPOS.matrix <- as.matrix(tdmPOS.tfidf) 
```
...
train2POS <- ovun.sample(label ~ ., data = trainPOS, method = "over")$data
table(train2POS$label)
```
...
corpusPOS <- Corpus(VectorSource(train2POS$text_a)) # turn into corpus
tdmPOS <- tm::DocumentTermMatrix(corpusPOS) 
tdmPOS.tfidf <- tm::weightTfIdf(tdmPOS)
tdmPOS.tfidf <- tm::removeSparseTerms(tdmPOS.tfidf, 0.999)  # sparsity being not well handled overall in R
tfidfPOS.matrix <- as.matrix(tdmPOS.tfidf) 
...
final$lbl <- as.factor(final$lbl) #Adding a vector of labels to the tfidf matrix changing the 0s to No and 1s to Yes

train2POS$lbl <- train2POS$label
avector <- as.vector(train2POS['lbl'])
finalPOS <- cbind(tfidfPOS.matrix, avector)
finalPOS <- as.data.frame(finalPOS) 
finalPOS$lbl=ifelse(finalPOS$lbl==0,"No","Yes") 
finalPOSl$lbl <- as.factor(finalPOS$lbl) #Adding a vector of labels to the tfidf matrix changing the 0s to No and 1s to Yes
...
fitSvm <- caret::train(lbl ~ ., data = final, method = 'svmLinear', scale=F, tuneLength = 5, metric = "F1", trControl=train.control) 
...
test.matrix.pos <- as.data.frame(tfidftest.matrix)
colsPOS <- colnames(finalPOS)
MissingPOS <- setdiff(colsPOS, names(test.matrix.pos))
test.matrix.pos[MissingPOS] <- 0
test.matrix.pos <- test.matrix.pos[colsPOS]
...
predSvm = predict(fitSvmPOS, newdata=test.matrix.pos)
