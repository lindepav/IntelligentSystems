...
```{r cluster2, echo=TRUE, cache=TRUE}
...
# 3. Modeling
```{r preprocess_test, echo=TRUE, cache=FALSE}
...
```{r analyze, echo=TRUE, cache=FALSE}
...
```{r feature-selection, echo=TRUE, warning=FALSE, cache=TRUE}
#install.packages('ggpubr')
library(mlr)
library(ggpubr)
finalRf <- create_model_data(tfidf.matrixRf, avectorRf)
final <- create_model_data(tfidf.matrixSVM, avectorSVM)
...
n_features <- c(head(features, 25), 'lbl')
train.data <- finalSVM[,n_features]

fitSvm <- caret::train(lbl ~ ., data = train.data, method = 'svmLinear',scale=F, tuneLength = 5, metric = "F1",
             trControl = train.control) 
fitRf <- caret::train(lbl ~ ., data = train.data, method = 'rf',tuneLength = 5, metric = "F1",
             trControl = train.control)
fitSvm$results
fitRf$results

test.matrix <- clean_test(train.data)
...
```{r nfeatures-visualize, echo=TRUE, cache=TRUE}
library(ggplot2)
features <- fv$data$name
