...
final <- cbind(tfidf.matrix[,highlyCorrelated], avector)
final <- as.data.frame(final)
final$lbl=ifelse(final$lbl==0,"No","Yes") 
final$lbl <- as.factor(final$lbl)

fit = glm(lbl~., data=final, family="binomial") #This doesnt complete for me but it should work
#fit <- train(label ~ ., data = train, method = 'bayesglm') #This works and takes little time but i cannot control it since
                                                            #there are different words in the test document
dat <- twoClassSim(200)
...
fit <- train(lbl ~ ., data = final, method = 'rf',tuneLength = 5, metric = "F1",
...
plotFilterValues(fv, sort = "dec", 10) 
```
...
# train the model
model <- train(lbl~., data=final, method="lvq", preProcess="scale", trControl=control)
